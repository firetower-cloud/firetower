import type { Event, Session, Step } from "~/api/generated/model";

/**
 * What a session did to come up, said in the conversation as it happens.
 *
 * The fold only — `ui/Bringup.tsx` draws it, because a list with a spine and a
 * turning mark is not the same object on a phone as it is in a window. The
 * labels and the state machine are shared, since "Fetching the repository"
 * should mean the same thing on every client.
 *
 ...
 */
export const DONE: Record<Step, string> = {
