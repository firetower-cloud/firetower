 ...
 * ## What a phone adds
 *
 * iOS suspends the process when you leave the app; the socket dies and there
 * is no arguing with it. So this closes cleanly on the way out and reopens on
 * the way back in, and every subscription resumes **from its own cursor** —
 * which is what `from` on `Sub` is for and what makes a reconnect replay
 * nothing and lose nothing. The cursor design was already right for this; all
 * that was missing was something to notice the app had gone away.
 *
 * Without it the phone came back to a socket that was open as far as
 * JavaScript knew and dead as far as the OS was concerned, and nothing arrived
 * again until something else forced a reconnect.
 *
...
import { wsBase, token } from "~/client/http";
...
export function SocketProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const open = () => {
      ...
      //
      // Built by hand, and passed as a string. React Native's `URL` has no
      // `searchParams`, and its `WebSocket` takes a string rather than a `URL`
      // — the browser accepts either, so the desktop's version does both
      // things this one cannot.
      const auth = token();
      const url =
        `${wsBase()}/api/v1/stream` + (auth ? `?t=${encodeURIComponent(auth)}` : "");
    ...
    /**
     * Backgrounded, the OS will suspend us and the socket with us.
     *
     * Closed on purpose rather than left to rot: a socket the OS killed while
     * we were away still reports `OPEN` when we come back, so `follow` sends
     * `sub` into a dead pipe and nothing ever arrives. Closing here means the
     * reopen below is a real one.
     */
    const lifecycle = AppState.addEventListener("change", (to: AppStateStatus) => {
      if (to === "active") {
        // Reopened from each subscription's own cursor, so nothing is replayed
    ...
    // Opened once for the life of the app, and reopened across suspensions.
    // Everything that varies — who is listening, where they got to — is read
    // through refs.
  }, []);
